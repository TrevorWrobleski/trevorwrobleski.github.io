...
-   **Kentucky Derby Color Palettes**: An R package offering over 25 color palettes inspired by the pageantry and tradition of the Kentucky Derby. Designed for data visualization in `ggplot2`, the collection includes qualitative, sequential, and diverging palettes drawn from jockey silks, Bluegrass landscapes, and bourbon heritage. The package features multiple colorblind-safe options, a complete `derby_theme()` for publication-ready graphics, and functions to easily explore and apply colors. The goal is to bring a touch of Kentucky tradition to data storytelling. [GitHub KYDerby](https://github.com/TrevorWrobleski/kentuckyderby/)
